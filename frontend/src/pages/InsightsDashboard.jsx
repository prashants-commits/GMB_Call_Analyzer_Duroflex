...
export default function InsightsDashboard() {
    ...
    const isOverCap = filteredCalls.length > 100;
    const isEmpty = filteredCalls.length === 0;

    // Build segment description for Gemini context
    const buildSegmentDesc = () => {
        const parts = [];
        if (selectedCities.length) parts.push(`Cities: ${selectedCities.join(', ')}`);
        if (selectedStores.length) parts.push(`Stores: ${selectedStores.join(', ')}`);
        if (selectedCategories.length) parts.push(`Categories: ${selectedCategories.join(', ')}`);
        if (selectedBarriers.length) parts.push(`Barriers: ${selectedBarriers.join(', ')}`);
    ...
    const handleGenerate = async () => {
        ...
        try {
            const res = await generateInsightsReport(cleanNumbers, buildSegmentDesc(), buildDateRange());
                ...
                <div className="bg-white p-6 rounded-[2rem] border border-slate-200 shadow-xl shadow-slate-200/50 mb-10 print:hidden">
                    ...
                    <div className="mt-6 pt-5 border-t border-slate-100 flex items-center justify-between">
                        <div className="flex items-center gap-6">
                            <div className="flex flex-col">
                                <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Selected Calls</span>
                                <span className={`text-3xl font-black leading-none mt-1 ${isOverCap ? 'text-red-500' : 'text-amber-600'}`}>
                            ...
                            {isOverCap && (
                                <div className="flex items-center gap-2 bg-red-50 border border-red-200 rounded-xl px-4 py-2 text-red-600 animate-pulse">
                                    ...
                                    <span className="text-xs font-bold">Sorry, you can select up to only 100 calls. Use filters to narrow down your search.</span>
                                </div>
                            )}
                        </div>
                ...
                {generating && (
                    <div className="flex flex-col items-center gap-6 py-16">
                        ...
                        <p className="text-slate-500 font-bold text-sm uppercase tracking-widest">
                            ✨ Gemini is analysing {filteredCalls.length} calls...
                        </p>
                        ...
                        <div className="flex items-center justify-between mb-8 print:mb-4">
                            <div>
                                ...
                                <p className="text-xs text-slate-400 font-bold uppercase tracking-widest mt-1">
                                    {buildSegmentDesc()} · {buildDateRange()} · {filteredCalls.length} calls analysed
                                </p>
                            </div>
                            <button
